...
for subj=1:length(IDs)
    ...
    % ruber left
    clear r c
    [r,c]=find(positionLR_sorted(:,1)==min(positionLR_sorted(:,1))); % find the voxel that are the most left
    transformed_landmark_coords{subj,1}.TopSlice.NucRuber_left = positionLR_sorted(r,:);
    
    % ruber right
    clear r c
    [r,c]=find(positionLR_sorted(:,1)==max(positionLR_sorted(:,1))); % find the voxel that are the most right
    transformed_landmark_coords{subj,1}.TopSlice.NucRuber_right = positionLR_sorted(r,:);
    
    % top curve
    clear r c
    [r,c]=find(positionLR_sorted(:,1)==median(positionLR_sorted(:,1),'all')); % find the voxel that are in the middle
    transformed_landmark_coords{subj,1}.TopSlice.TopBrainstem = positionLR_sorted(r,:);
    ...
    % brainstem bottom
    transformed_landmark_coords{subj,1}.BottomSlice.BottomBrainstem = positionLR_sorted;
