...
% load the landmark image drawn on MNI: download example version from here
% (https://github.com/alex-yi-writes/LC-SpatialTransformation2021/blob/350a7c01950b8183f1b3ba8cbee7ddafc4faed0c/MNI_landmarks_v7_2Labels.nii)
...
for subj=1:length(IDs)
    ...
    % periaqueductal grey
    transformed_landmark_coords{subj,1}.TopSlice.PeriaqueductalGrey = [median(positionLR_sorted(:,1)) median(positionLR_sorted(:,2)) slices(1)]; %ceil( median(cluster3) );
    ...
    % brainstem bottom
    transformed_landmark_coords{subj,1}.BottomSlice.PerifastigialSulcus = [median(positionLR_sorted(:,1)) median(positionLR_sorted(:,2)) slices(3)]; %ceil( median(positionLR_sorted) );
